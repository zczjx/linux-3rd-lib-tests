...
*                
* @filename: libv4l_k30_4412_jpg_enc.c 
*                
...
int main(int argc, char **argv)
{
	...
	snapshot_run = 1;
	err = pthread_create(&snap_tid, NULL, snapshot_thread, NULL);
...
static void *snapshot_thread(void *arg)
{
	...
	while(1 == snapshot_run)
	{
		...
		if('s' == cmd)
		{
			...
			write(fd_img, out_buf.start[0], out_buf.length[0]);
			close(fd_img);
		}
